<<<<<<< Updated upstream:src/torcs/DefaultDriverAlgorithm.java
package torcs;
...
public class DefaultDriverAlgorithm extends AbstractAlgorithm {
    ...
    public void run(boolean continue_from_checkpoint) {
        if(!continue_from_checkpoint){
            ...
            race.setTrack( AbstractRace.DefaultTracks.getTrack(DefaultDriver.trackNumber));
            race.laps = DefaultDriver.numberOfLaps;

            //for speedup set withGUI to false
            results = race.runRace(drivers, DefaultDriver.withGIU);
...
public class DefaultDriverAlgorithm extends AbstractAlgorithm {
    ...
    public void run(boolean continue_from_checkpoint) {
        if(!continue_from_checkpoint){
            ...
            //Start a race
            DefaultRace race = new DefaultRace();
            race.setTrack( AbstractRace.DefaultTracks.getTrack(2));
            race.laps = 1;
    ...
    public static void main(String[] args) {
        ...
        //Set path to torcs.properties
        TorcsConfiguration.getInstance().initialize(new File("Resources/torcs.properties"));
		/*
		 *
		 * Start without arguments to run the algorithm
		 * Start with -continue to continue a previous run
...
>>>>>>> Stashed changes:src/RaceFiles/DefaultDriverAlgorithm.java
}